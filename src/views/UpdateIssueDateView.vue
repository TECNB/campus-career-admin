        ...
        <el-scrollbar>
            ...
            <div class="flex justify-center items-center gap-3">
                <div class="flex-[2_1_0] min-h-96 bg-white rounded-2xl m-10 p-10">
                    ...
                    <p class="text-left mt-5">发行计划名称</p>
                    <!-- 下面为发行计划名称搜索框 -->
                    <el-input v-model="name" placeholder="请输入发行计划名称" class="mt-4">
                        <template #prefix>
                    ...
                    <p class="text-left mt-5">发行计划介绍</p>
                    <!-- 下面为发行计划名称搜索框 -->
                    <el-input v-model="shortDescription" placeholder="请输入发行计划介绍" class="mt-4">
                        <template #prefix>
                            <el-icon color="var(--text-100)" class="el-input__icon">
                                <CollectionTag />
                            </el-icon>
                    ...
                    <p class="text-left my-5">发行计划时间</p>
                    <el-date-picker v-model="issueTime" type="datetime" value-format="x" placeholder="点击选择发行计划时间" />

                </div>
                <div class="flex-[1_1_0] min-h-96 bg-white rounded-2xl m-10 p-10">
                    <p class="text-left font-medium">发行计划图片</p>
...
const handleAddType = async () => {
    ...
    if (uploadedImage.value) {
        ...
        await addIssueDates(formData).then((res) => {
            console.log(res);
            ElMessage.success('添加发行计划成功');
            router.push('/issueDate');
        }).catch((err) => {
            console.log(err);
            ElMessage.error('添加发行计划失败,请重试');
        })
    } else {
        ElMessage.error('请上传发行计划图片');
    }
};
// 编辑发行计划
const handleEditType = async () => {
    ...
    if (uploadedImage.value) {
        ...
        //     console.log(res);
        //     ElMessage.success('编辑发行计划成功');
        //     router.push('/type');
        // }).catch((err) => {
        //     console.log(err);
        //     ElMessage.error('编辑发行计划失败,请重试');
        // })
    } else {
        ElMessage.error('请上传发行计划图片');
    }
