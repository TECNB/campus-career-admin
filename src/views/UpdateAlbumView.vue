        ...
        <el-scrollbar>
            ...
            <div class="flex justify-center items-center gap-3">
                <div class="flex-[2_1_0] min-h-96 bg-white rounded-2xl m-10 p-10">
                    ...
                    <p class="text-left mt-5">合集名称</p>
                    <!-- 下面为合集名称搜索框 -->
                    <el-input v-model="name" placeholder="请输入合集名称" class="mt-4">
                        <template #prefix>
                ...
                <div class="flex-[1_1_0] min-h-96 bg-white rounded-2xl m-10 p-10">
                    <p class="text-left font-medium">合集图片</p>
...
import { getAlbumById,editAlbum } from "../api/album";
...
let album: Ref<Album | null> = ref(null);
...
onMounted(async () => {
    ...
    if (route.params.id === "create") {
        ...
        loading.value = true;
        // 获取合集信息
        await getAlbumById(route.params.id as string).then((res) => {
            album.value = res;
            name.value = res.name;
...
const handleAddType = async () => {
    ...
    if (uploadedImage.value) {
        ...
        await addIssueDates(formData).then((res) => {
            console.log(res);
            ElMessage.success('添加合集成功');
            router.push('/issueDate');
        }).catch((err) => {
            console.log(err);
            ElMessage.error('添加合集失败,请重试');
        })
    } else {
        ElMessage.error('请上传合集图片');
    }
};
// 编辑合集
const handleEditType = async () => {
    ...
    if (uploadedImage.value) {
        ...
        formData.append('intro', album.value!.intro);
        formData.append('finalCollection', album.value!.finalCollection);

        await editAlbum(route.params.id as string, formData).then((res) => {
            console.log(res);
            ElMessage.success('编辑合集成功');
            router.push('/album');
        }).catch((err) => {
            console.log(err);
            ElMessage.error('编辑合集失败,请重试');
        })
    } else {
        ElMessage.error('请上传合集图片');
    }
