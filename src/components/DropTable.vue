        ...
        <div class="body">
            ...
            会触发handleQuery方法，handleQuery方法通常用于执行搜索操作。-->
                <div class="SearchInput">
                    ...
                    <input type="text" placeholder="请输入空投名称">
                </div>
                <div class="FilterBox">
                    <el-icon>
                        <Operation />
            ...
            <el-scrollbar height="100%">
                <el-table v-loading="loading" :data="tableData" class="tableBox" table-layout="fixed"
                    ...
                    <el-table-column prop="collectionId" label="空投对象">
                        <template v-slot="{ row }">
                            {{ collectionName===null ? row.collectionId:collectionName}}
                        </template>
                    </el-table-column>

                    <el-table-column prop="dropCount" label="投放数量"></el-table-column>

                    <el-table-column prop="dropTime" width="160" label="开始时间">
                        <template v-slot="{ row }">
                            <!-- 解析日期，格式如下：2024-04-01T16:33:00.127+08:00,精确到秒 -->
                            {{ new Date(row.dropTime).toLocaleString() }}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="200" align="center">
                        <template v-slot="{ row }">
                            <el-button text bg type="success" size="small" @click="promotion(row.id)">
                                编辑
                            </el-button>
...
const page = ref(1);
const input = ref('');
const tableData: Ref<Drop[]> = ref([]);
const counts = ref(tableData.value.length);
const loading = ref(false);
const allData = ref<Drop[]>([]);
...
onMounted(async () => {
    ...
    for (let i = 0; i < allData.value.length; i++) {
        await getCollectionById(allData.value[i].collectionId).then((res) => {
            collectionName.value = res.name;
